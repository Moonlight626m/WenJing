...
> 本文档的 `#N` 为规划编号（#1–#14 为原始规划波次，多用户化从 #16 起）。
> GitHub tracker 现状：本仓库仅存在 #16–#26（#15 为 PR）；#1–#14 未建票，
> 其中 #14「内容质量 Gold Set」为唯一未实现的规划项（尚未建票）。
> spec: 交付可投入真实试用的文境核心 MVP。
...
### [#14] 课文内容质量 Gold Set 与人工评分门禁 ⬜ 未开始（规划项，尚未建 GitHub 票）
...
### [#24] 学生端 UI：广场 / 详情 / 游玩 / 我的游戏 ✅ 已完成（2026-09-16）

- [x] 广场（org + public，可搜索）→ 详情 → 开局/选角/游玩（复用消息流/交互卡/回溯）
- [x] 我的游戏续玩与回溯；刷新/断线重连恢复；公开剧本仅登录可见
- [x] `core-flow.spec.ts` 取消 skip；E2E 覆盖登录→开局→游玩全流程

### [#25] 运营后台 UI ✅ 已完成（2026-09-16）

- [x] `/admin` 只读看板：剧本库（org/status 筛选）+ token 用量聚合表（org/purpose/时间筛选 + 合计）
- [x] 非 super_admin 拒绝（layout+page 双保险）；看板正文无任何按钮/写入口（E2E 断言）
- [x] E2E：admin.spec.ts（看板筛选、越权、未登录）
- 注：「用户 / 组织」面板因 #22 无对应端点未做，已在 issue 登记缺口与后续工单建议

### [#26] 文档、术语与部署收尾 ✅ 已完成（2026-09-16）
