...
import type {
  GenerationPhaseStateValue,
  ScriptDetail,
  ScriptVisibility,
} from "@/lib/contracts/types";

const PHASE_DOT: Record<GenerationPhaseStateValue, string> = {
        ...
        <section className="flex flex-col gap-2 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <ul className="flex flex-col gap-1">
            {generation.phases.map((phase) => (
              ...
              >
                <span
                  className={`inline-block h-1.5 w-1.5 rounded-full ${PHASE_DOT[phase.state]}`}
                />
                {PHASE_LABELS[phase.name] ?? phase.name}
                {phase.detail && <span className="text-zinc-400">· {phase.detail}</span>}
              </li>
            ))}
          </ul>
        </section>
